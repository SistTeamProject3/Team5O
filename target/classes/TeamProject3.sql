...
BC)
CC)
WHERE (N_STEP = 0 AND N_EVENT_SEQ = 3 ) 
...
(SELECT BC.* FROM (
SELECT AC.* FROM (
SELECT A.*, B.E_START_DATE, B.E_TITLE FROM TP3_NEWSFEED A INNER JOIN TP3_EVENT B ON A.N_EVENT_SEQ=B.E_SEQ ORDER BY A.N_SEQ DESC) 
AC) 
BC)
CC)
WHERE (N_STEP = 0 AND N_EVENT_SEQ = 3 ) 
